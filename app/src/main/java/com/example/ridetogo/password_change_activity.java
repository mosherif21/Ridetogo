...
public class password_change_activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //click listener for new password save
        btn_password_save.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //if new password is less than 8 characters display error textview
                if (new_password.getText().toString().trim().length() < 8) {
                    ...
                    //if old password is less than 8 characters than it must be incorrect
                    if (old_password.getText().toString().trim().length() < 8) {
                        password_error.setText("old password is incorrect");
                        password_error.setVisibility(View.VISIBLE);
                    } else {
                        //re-authenticate with old password and user email to check if old password entered is incorrect
                        ...
                        user.reauthenticate(credential).addOnCompleteListener(new OnCompleteListener<Void>() {
                            public void onComplete(@NonNull Task<Void> task) {
                                if (task.isSuccessful()) {
                                    //if authentication successful then update password with new password
    ...
    public void finish() {
        super.finish();
        //override activity finish to animate from up to down
